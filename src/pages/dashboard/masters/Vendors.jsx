...
import { Plus, Search, Pencil, Trash2, Phone, MessageCircle, Filter, Sparkles, Building2, Users, Clock } from 'lucide-react'
...
function Stars({ rating }) {
  return (
    <span style={{ letterSpacing: '1px', fontSize: '13px' }}>
      {[1,2,3,4,5].map(s => (
        <span key={s} style={{ color: s <= rating ? 'var(--icon-booking)' : 'var(--dash-border)' }}>★</span>
      ))}
    </span>
  )
}
...
export default function Vendors() {
  ...
  const { page, setPage, totalPages, paginated } = usePagination(filtered, 6, catFilter + search)
    ...
    <div style={{ padding: '28px 32px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          borderBottom: '1.5px solid var(--dash-border)', marginBottom: '14px',
        }}>
          ...
          <Link to="/dashboard/masters/vendors" style={{
            ...
            <Users size={15} style={{ color: 'var(--icon-booking)' }} /> Vendor Master
          </Link>
        ...
        <div style={{
          ...
          flexWrap: 'wrap', gap: '14px', background: 'var(--dash-card-bg)', padding: '14px 18px',
          borderRadius: '14px', border: '1.5px solid var(--dash-border)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px', flexWrap: 'wrap', flex: 1 }}>
            <div style={{
              background: 'var(--btn-ghost-bg)', padding: '6px 14px', borderRadius: '8px',
              fontSize: '12.5px', fontWeight: 650, color: 'var(--dash-text-secondary)', display: 'inline-flex', alignItems: 'center', gap: '6px',
              whiteSpace: 'nowrap', border: '1px solid var(--btn-ghost-border)',
            }}>
              <span>{items.length} vendors</span>
              <span style={{ opacity: 0.4 }}>•</span>
              <span style={{ color: 'var(--badge-confirmed)' }}>{availCount} available</span>
            </div>

            <div style={{ position: 'relative', minWidth: '220px', maxWidth: '300px', flex: 1 }}>
              <Filter size={13} style={{ position: 'absolute', left: '11px', top: '50%', transform: 'translateY(-50%)', color: 'var(--icon-booking)', pointerEvents: 'none' }} />
              <input
            ...
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
              {['All', 'Makeup Artist','Hair Stylist', 'Photographer', 'Saree Draper', 'Mehendi Artist'].map(c => (
                <button key={c} onClick={() => setCatFilter(c)} style={{
                  ...
                  ...(catFilter === c
                    ...
                    : { background: 'var(--dash-filter-wrap)', color: 'var(--dash-filter-muted-tx)', border: '1px solid var(--dash-border-subtle)' }
                  ),
                }}>{c}</button>
              ))}
            </div>
        ...
        <div style={{ overflowX: 'auto' }}>
                  ...
                  >
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <div>
                          ...
                          {item.notes && <div style={{ fontSize: '11px', color: 'var(--dash-text-muted)' }}>{item.notes}</div>}
                        </div>
                      </div>
                    </td>
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        <span style={{ fontSize: '12px', color: 'var(--dash-text-primary)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                          <Phone size={11} style={{ color: 'var(--icon-booking)' }} /> {item.contact}
                        </span>
                      </div>
                    </td>

                    {/* Charges */}
                    <td style={{ padding: '14px 16px', fontWeight: 600, color: 'var(--icon-booking)', whiteSpace: 'nowrap' }}>
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      <div style={{ display: 'flex', gap: '6px' }}>
                        <button onClick={() => openWhatsApp(item)} title="WhatsApp" style={{
                          ...
                          background: 'rgba(37,211,102,0.1)', cursor: 'pointer',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                        }}>
                          <MessageCircle size={13} style={{ color: '#25D366' }} />
                        </button>
                        <button onClick={() => openEdit(item)} title="Edit" style={{
                          width: '30px', height: '30px', borderRadius: '8px', border: '1.5px solid var(--btn-ghost-border)',
                          background: 'var(--btn-ghost-bg)', cursor: 'pointer',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                        }}>
                          <Pencil size={13} style={{ color: 'var(--icon-booking)' }} />
                        </button>
                        <button onClick={() => handleDelete(item.id)} title="Delete" style={{
                          width: '30px', height: '30px', borderRadius: '8px', border: '1.5px solid var(--badge-rejected-bg)',
                          background: 'var(--badge-rejected-bg)', cursor: 'pointer',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                        }}>
                          <Trash2 size={13} style={{ color: 'var(--badge-rejected)' }} />
                        </button>
                      </div>
                    </td>
                  </tr>
          ...
          {filtered.length === 0 && (
            <EmptyState
              ...
              subtitle={search ? `No vendor matches "${search}". Try adjusting your search or category filter.` : 'Add your first vendor or team member to start managing team availability.'}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            ...
            <CustomSelect
              label="Rating"
              value={form.rating}
              options={[5,4,3,2,1].map(r => ({ value: r, label: `${r} Star${r !== 1 ? 's' : ''}` }))}
